...
editor_ui_all :: proc(editor: ^Editor, scratch: oc.arena_scope) {
  ...
  editor_ui_filters(ui, stylesheet, "States", &editor.filters_state, editor.all_states)
  editor_ui_filters(ui, stylesheet, "Tags", &editor.filters_tag, editor.all_tags)

  style: Style
  style_sizex_full(&style)
  style_after(style)
  oc.ui_label_str8("Random Selected")

  filter_sig := filter_button(ui, stylesheet, "RNG", editor.filter_single != nil)
  if filter_sig.pressed {
...
filter_button :: proc(ui: ^oc.ui_context, stylesheet: ^Stylesheet, label: string, selected: bool) -> oc.ui_sig {
  style := stylesheet_fetch(stylesheet, selected ? "button_filter1" : "button_filter0")
  box := style_box_make(ui, label, style, {}, {}, { .CLICKABLE, .DRAW_TEXT })
...
editor_ui_filters :: proc(ui: ^oc.ui_context, stylesheet: ^Stylesheet, filter_name: string, filters: ^map[string]bool, unique: map[string]int) {
  ...
  sig := filter_button(ui, stylesheet, key, selected)
