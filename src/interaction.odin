...
editor_ui_all :: proc(editor: ^Editor, scratch: oc.arena_scope) {
...

	{
		...
		editor_ui_filters(&editor.ui_context, "States", &editor.filters_state, editor.all_states)
		editor_ui_filters(&editor.ui_context, "Tags", &editor.filters_tag, editor.all_tags)
...
task_line_begin :: proc(index: int) {
	style: Style
	style_layout(&style, .X, {}, TASK_GAP)
	// style_animate(&style, 0.5, style.mask)
	// style_sizex_full(&style, 1)
	// style_border(&style, 5, oc.color_rgba(1, 0, 0, 1))
	style_next(style)
	test := fmt.tprintf("padded %d", index)
	oc.ui_box_begin_str8(test, style.box)
}

editor_ui_filters :: proc(ui: ^oc.ui_context, filter_name: string, filters: ^map[string]bool, unique: map[string]int) {
	style: Style
	style_sizex_full(&style)
	style_after(style)
	...
	for key, value in unique {
		...
		style = {}
		style_bg_color(&style, selected ? oc.color_rgba(0.5, 0.1, 0.1, 1) : ui.theme.primary)
		style_color(&style, selected ? oc.color_rgba(0.05, 0.05, 0.05, 1) : ui.theme.text0)
		style_sizey(&style, ed.font_size + 10)
		style_animate(&style, 0.5, style.mask)
		style_sizex_full(&style, 1)
		style_align(&style, .CENTER, .CENTER)
		style.box += { .CLICKABLE, .DRAW_TEXT }
		style_next(style)

		box := oc.ui_box_make_str8(key, style.box)
...
editor_task_text_display :: proc(editor: ^Editor) {
	...
	style: Style
	style_sizex_full(&style)
	style_sizey(&style, 40)
	color := editor.ui_context.theme.bg0
	color.a *= 1
	style_bg_color(&style, color)
	style.box += { .OVERLAY }
	style_next(style)
	oc.ui_container("text display", style.box)

	style = style_fullsize()
	style_align(&style, .CENTER, .CENTER)
	color = editor.ui_context.theme.text0
...
editor_task_menu :: proc(editor: ^Editor, scratch: oc.arena_scope) {
	...
	style_inner: Style
	style_layout(&style_inner, .Y, 50, 30)
	...
	{
		style = style_fullsize()
		style_layout(&style, .Y, 0, 5)
		style_sizex(&style, 500)
		style_next(style)
