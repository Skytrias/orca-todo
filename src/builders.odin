...
custom_button_old :: proc(ui: ^oc.ui_context, label: string) -> oc.ui_sig {
	theme := ui.theme
	style: Style
	style_text(&style)
	style_align(&style, .CENTER, .CENTER)
	style_margin(&style, 12, 6)
	style_color(&style, theme.primary)
	style_bg_color(&style, theme.fill0)
	style_roundness(&style, theme.roundnessSmall)
	style_next(style)

	style_hover: Style
	style_bg_color(&style_hover, theme.fill1)
	style_before_on_hover(&ui.frameArena, style_hover)

	style_active: Style
	style_bg_color(&style_active, theme.fill2)
	style_before_on_hover_active(&ui.frameArena, style_active)
...
custom_button_v2 :: proc(ui: ^oc.ui_context, stylesheet: ^Stylesheet, label: string) -> oc.ui_sig {
	style_push("button")
	defer style_pop("button")
	box := style_box_make(ui, label, { .CLICKABLE })
	return button_behaviour(box)
}